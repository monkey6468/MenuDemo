...
@objc class SWMenuController: UIView {
    ...
    func update() {
        ...
        menuItems.forEach { (item) in
            ...
            var itemWidth = title.boundingRect(with: CGSize(width: CGFloat.infinity,
                                               ...
                                               attributes: [ NSAttributedStringKey.font : kMenuFont ],
                                               context: nil).width + 20
            if itemWidth < kMenuMinWidth {
                itemWidth = kMenuMinWidth
            }
            totalWidth += itemWidth
            if !menuWidthList.isEmpty {
                totalWidth += kMenuSpace
            }
        ...
        if menuItems.count == 2 {
            let width = menuWidthList[1]
            let menuLineR = width + kMenuSpace
            arrowView.frame = CGRect(x: menuLineR ,
        ...
        if totalWidth > kContentViewMaxWidth {
            ...
            for (i, menuWidth) in tempList.enumerated() {
                ...
                if accumulatedWidth > pageMaxWidth || i == menuWidthList.count - 1 {
                    ...
                    for index in startIndex..<endIndex {
                        ...
                        let line = produceMenuLine()
                        line.frame = CGRect(x: buttonX+width, y: 0, width: kMenuSpace, height: kMenuHeight)
                        pageView.addSubview(line)
            ...
            for (i, item) in menuItems.enumerated() {
                ...
                let line = produceMenuLine()
                line.frame = CGRect(x: buttonX+menuWidthList[i], y: 0, width: kMenuSpace, height: kMenuHeight)
                pageView.addSubview(line)
                menuLines.append(line)
    ...
    private func produceMenuLine() -> UIView {
        let line = UIView()
